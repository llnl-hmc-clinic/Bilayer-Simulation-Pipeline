...
def analyze(dirList, resultsFP):
	...
	for runDir in dirList:
		...
		fp = "/p/lustre1/muntere/fatslim-0.2.1/fatslim"
		subprocess.call(["python3", fp, "membranes", "-c", groFP,  "-n", headgroupsFP, "--output-index", runDir + "/analysis/bilayer_leaflet.ndx"])	
		subprocess.call(["python3", fp, "thickness", "-c", groFP, "-n", headgroupsFP, "-t", xtcFP, "-b", str(1.9 * finalSimulationTime / 2), "-e", str(finalSimulationTime), "--plot-thickness", runDir + "/analysis/thickness.xvg", "--nthreads", "12"])
		subprocess.call(["python3", fp, "apl", "-c", groFP, "-n", headgroupsFP, "-t", xtcFP, "-b", str(1.9 * finalSimulationTime / 2), "-e", str(finalSimulationTime), "--plot-apl", runDir + "/analysis/apl.xvg", "--plot-area", runDir + "/analysis/area.xvg", "--nthreads", "12"])
		...
		fp = '/p/lustre1/muntere/GROMACS_LS_Beta/bin/tensortools'
		command =f"python3 {fp} -f {runDir}/20fs/stress/localstress.dat0 --prof z -o {runDir}/20fs/stress/stress_z.txt"                
		subprocess.call(command, shell=True)
